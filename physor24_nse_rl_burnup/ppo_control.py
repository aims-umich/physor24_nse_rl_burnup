...
model_path= './ppo_nsteps_300/ppo_best_model.pkl'             #test mode

#-------------------------------------------------------------
# Wrapper function to construct a vectorized environment
#--------------------------------------------------------------

def make_env(env_id: str, surr_path: str, action_type: str, rank: int, seed: int = 42) -> Callable:
    """
...
if __name__ == '__main__':
  ...
  casename='ppo'                               #prefix for naming convention
  load_model=False
  ...
  if 1:
    ...
    #n_ppo_steps=[200, 300, 400]           # the larger the value, the longer the time the policy gets updated. 
    ...
    grid=list(product(n_ppo_steps, ent_coef, vf_coef, clip_range))   #this forms a multidimensional grid for search
    ...
    for k in grid: 
      ...
      if run_mode == 'train':
          ...
          ppo_env = SubprocVecEnv([make_env(os.path.join(dir_name, casename), surr_path, action_type, i) for i in range(num_cpu)])
          ...
          t0=tme.time()
          #create a ppo object
          ppo = PPO(MultiInputPolicy, ppo_env, n_steps=k[0], ent_coef=k[1], vf_coef=k[2], clip_range=k[3], seed=42, verbose=0, device='cpu')
          #train ppo, this line takes time
          if load_model:
            ppo=ppo.load('./ppo_nsteps_300_newrwd/ppo_best_model.pkl', env=ppo_env)
              
          ppo.learn(total_timesteps=total_timesteps, log_interval=10, callback=callback)
          #training time
          tend=tme.time()
          ...
          #****************************** 
          # Create a vectorized environment
          ppo_env = SubprocVecEnv([make_env(casename, surr_path, action_type, i) for i in range(2)])
          ...
          for s in range(n_samples):
              ...
              obs, rewards, dones, info = ppo_env.step(action)
              tend=tme.time()
